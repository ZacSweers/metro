...
internal class ContributionsFirGenerator(session: FirSession, compatContext: CompatContext) :
  ...
  override fun getTopLevelClassIds(): Set<ClassId> {
    ...
    for (contributingClass in contributingClasses) {
      // Skip classes generated by Metro extensions — FIR can't generate top-level classes
      // from other generated top-level classes, so contribution providers can't work for them.
      @OptIn(SymbolInternals::class)
      if (contributingClass.fir.isExtensionGenerated == true) continue
