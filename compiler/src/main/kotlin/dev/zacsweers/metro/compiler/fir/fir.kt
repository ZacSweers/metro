 ...
 * containers. This happens when [MetroOptions.generateContributionProviders] is enabled, or when
 * [MetroOptions.bindingContributionsAsContainers] is enabled for pure binding contributions.
 *
 * This predicate is intentionally broader than the public `generateContributionProviders` option.
 * Callers that need the public-provider semantics of that option, such as public holder visibility
 * or suppressing direct injectable factories, should still check it separately.
 *
 * Top-level classes generated by Metro extensions are never routed through this path, because FIR
 * cannot generate top-level classes from other generated top-level classes. These classes are
 * tagged with [isExtensionGenerated] by the [CompositeMetroFirDeclarationGenerationExtension].
 */
@OptIn(SymbolInternals::class)
internal fun FirBasedSymbol<*>.usesContributionProviderPath(session: FirSession): Boolean {
  val contributionAnnotations =
    annotationsIn(session, session.classIds.allContributesAnnotationsWithContainers).toList()
  val hasBindingContribution = contributionAnnotations.any {
    it.toAnnotationClassIdSafe(session) in
      session.classIds.contributesBindingLikeAnnotationsWithContainers
  }
  if (!hasBindingContribution) return false

  val options = session.metroFirBuiltIns.options
  val hasContributesTo = contributionAnnotations.any {
    val annotationClassId = it.toAnnotationClassIdSafe(session)
    annotationClassId in session.classIds.contributesToAnnotations ||
      annotationClassId?.parentClassId?.let { parentClassId ->
        parentClassId in session.classIds.contributesToAnnotations
      } == true
  }
