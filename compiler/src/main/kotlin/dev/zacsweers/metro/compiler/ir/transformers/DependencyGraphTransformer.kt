...
/** Prepared nodes keep compiler lookups and IR changes on the main compiler thread. */
private class PreparedGraph(
  val children: List<PreparedGraph>,
  val prepare: (List<ValidationResult>) -> PreparedGraphValidation,
)

/** Only the completed binding graph's analysis runs on a worker. */
private class PreparedGraphValidation(
  val seal: IrBindingGraph.PreparedSeal,
  val finish: (IrBindingGraph.BindingGraphResult) -> ValidationResult,
)

/** Combines a child's validation result with the keys it requests from its parent. */
...
internal class DependencyGraphTransformer(
   ...
   * doesn't reference classes generated later. Run the merger here in IR and append the merged
   * supertypes onto the generated `$$Impl` so the binding graph builder picks them up via
   * `allSupertypesSequence`.
   */
  internal fun applyIrContributionMergeIfNeeded(
    ...
    if (contributions.supertypes.isEmpty()) {
      return
    }
   ...
   *
   * Prepares the entire tree, analyzes it bottom-up, then generates parent-first so children can
   * resolve property tokens against finalized parent properties.
   */
  private fun processGraphInner(
    ...
    // Children determine which parent bindings must be kept before the parent is analyzed.
        ...
        trace("Prepare and validate") {
          val prepared =
            prepareDependencyGraph(
              ...
              dependencyGraphAnno,
              metroGraph,
              parentContextReader = null,
              diagnosticTag,
            )
          analyzeGraphTree(prepared)
        }

      if (validationResult.hasErrors) {
        validationResult.sealResult
      } else {
        // Parent properties must exist before child implementations can reference them.
  ...
  /**
   * Prepares a complete extension tree before analysis starts. Child results determine parent
   * roots, so binding population and analysis proceed in bottom-up batches afterward.
   *
   * Generated nodes stay attached to this tree through parent-first code generation.
   */
  context(traceScope: TraceScope)
  private fun prepareDependencyGraph(
    ...
    val childGraphs = mutableListOf<PreparedGraph>()
    var finishChildren: (List<ValidationResult>) -> Unit = {}
    ...
    if (node.graphExtensions.isNotEmpty()) {
      ...
      // Each child records its parent requests separately during graph preparation.
      val snapshot = localParentContext.snapshot()
      val collectors =
        node.graphExtensions.entries.map { (contributedGraphKey, _) ->
          val contributedGraph = prebuiltExtensionGraphs.getValue(contributedGraphKey)
          val collector = UsedKeyCollector()
          val childTag = contributedGraph.kotlinFqName.shortName().asString()
          childGraphs +=
            trace("[$childTag] Prepare child graph") {
              prepareDependencyGraph(
      ...
      finishChildren = { childValidationResults ->
        val extensionTasks =
          node.graphExtensions.entries.mapIndexed { index, (key, accessors) ->
            ExtensionValidationTask(
              contributedGraphKey = key,
              contributedGraph = prebuiltExtensionGraphs.getValue(key),
              accessor = accessors.first().accessor,
    ...
    return PreparedGraph(childGraphs) { childValidationResults ->
      finishChildren(childValidationResults)
      ...
      val preparedSeal =
        bindingGraph.prepareSeal(childGraphScopes) { errors ->
          errors
      ...
      PreparedGraphValidation(preparedSeal) { sealResult ->
        ...
        if (hasErrors) {
          ...
          return@PreparedGraphValidation validationResult
        }
  ...
  /** Every batch joins before the pipeline can populate more bindings or touch compiler state. */
  private fun analyzeGraphTree(root: PreparedGraph): ValidationResult {
    val levels = mutableListOf<MutableList<PreparedGraph>>()
    val pending = ArrayDeque<Pair<PreparedGraph, Int>>()
    pending.addLast(root to 0)
    while (pending.isNotEmpty()) {
      val (graph, depth) = pending.removeFirst()
      if (depth == levels.size) {
        levels.add(mutableListOf())
      }
      levels[depth].add(graph)
      for (child in graph.children) {
        pending.addLast(child to depth + 1)
      }
    }

    val results = mutableMapOf<PreparedGraph, ValidationResult>()
    for (level in levels.asReversed()) {
      val prepared = level.map { graph -> graph.prepare(graph.children.map(results::getValue)) }
      val analyses =
        if (forkJoinPool != null) {
          prepared.parallelMap(forkJoinPool) { it.seal.analyze() }
        } else {
          prepared.map { it.seal.analyze() }
        }
      for ((index, graph) in level.withIndex()) {
        val task = prepared[index]
        results[graph] = task.finish(task.seal.finish(analyses[index]))
      }
    }
    ...
    val reports = unusedGraphInputs.map { unusedBinding ->
      ...
      val notes = buildList {
        ...
        unusedBinding.typeKey.type.rawTypeOrNull()?.let { containerClass ->
          // Intermediate containers may have been visited without caching their own closure.
          val transitivelyIncluded =
            bindingContainerResolver.resolve(containerClass).mapToSet { it.typeKey }
