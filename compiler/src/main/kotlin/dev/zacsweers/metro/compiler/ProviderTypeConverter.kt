...
internal class MetroProviderFramework(
  private val metroFrameworkSymbols: Symbols.MetroFrameworkSymbols
) : ProviderFramework {
 ...
 * - jakarta.inject.Provider
 */
internal class DaggerProviderFramework(private val symbols: Symbols.DaggerSymbols) :
  ProviderFramework {
  ...
  override fun IrExpression.handleSameFramework(targetKey: IrContextualTypeKey): IrExpression {
    ...
    if (targetClassId == Symbols.DaggerSymbols.ClassIds.DAGGER_LAZY_CLASS_ID) {
      return provider.toLazy(targetKey)
    }
  ...
  override fun IrExpression.fromMetroProvider(targetKey: IrContextualTypeKey): IrExpression =
    with(scope) {
      ...
      val conversionFunction =
        when (targetClassId) {
          Symbols.DaggerSymbols.ClassIds.DAGGER_INTERNAL_PROVIDER_CLASS_ID ->
            symbols.asDaggerInternalProvider
          Symbols.DaggerSymbols.ClassIds.JAVAX_PROVIDER_CLASS_ID -> symbols.asJavaxProvider
          Symbols.DaggerSymbols.ClassIds.JAKARTA_PROVIDER_CLASS_ID -> symbols.asJakartaProvider
        ...
        provider.type.rawTypeOrNull()?.let { rawType ->
          rawType.getAllSuperTypes(excludeSelf = false, excludeAny = true).firstNotNullOfOrNull {
            ...
            when (type.classOrNull?.owner?.classId) {
              ...
              Symbols.DaggerSymbols.ClassIds.JAVAX_PROVIDER_CLASS_ID -> lazyFromJavaxProvider
              Symbols.DaggerSymbols.ClassIds.JAKARTA_PROVIDER_CLASS_ID -> lazyFromJakartaProvider
