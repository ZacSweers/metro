...
public class LatticeCommandLineProcessor : CommandLineProcessor {
  ...
  override val pluginOptions: Collection<AbstractCliOption> =
    LatticeOption.entries.map { it.raw.cliOption }
  ...
  override fun processOption(
    ...
    when (val latticeOption = LatticeOption.entriesByOptionName[option.optionName]) {
      null -> error("Unknown plugin option: ${option.optionName}")
      else -> with(latticeOption.raw) { configuration.put(value) }
    }
  }
}
