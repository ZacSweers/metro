...
internal object MembersInjectChecker : FirClassChecker(MppCheckerKind.Common) {
  override fun check(declaration: FirClass, context: CheckerContext, reporter: DiagnosticReporter) {
    ...
    for (callable in declaration.declarations.filterIsInstance<FirCallableDeclaration>()) {
      ...
      val annotations = callable.symbol.latticeAnnotations(session)
      if (!annotations.isInject) continue

      if (!isInClass) {
        reporter.reportOn(
          callable.source,
          FirLatticeErrors.MEMBERS_INJECT_ERROR,
        ...
        reporter.reportOn(
          ...
          FirLatticeErrors.MEMBERS_INJECT_STATUS_ERROR,
      ...
      if (
        ...
        reporter.reportOn(
          callable.source,
          FirLatticeErrors.MEMBERS_INJECT_WARNING,
          "Non-null injected member property in constructor-injected class should usually be moved to the inject constructor. If this has a default value, use Lattice's default values support.",
          context,
        )
      }
        ...
        callable.returnTypeRef.coneTypeOrNull?.let {
          if (!it.isUnit) {
            reporter.reportOn(
              ...
              FirLatticeErrors.MEMBERS_INJECT_RETURN_TYPE_WARNING,
